...
class BasicLSTMCell(RNNCell):
  ...
  def call(self, inputs, state):
    ...
    if self._linear is None:
      ...
      self._linear = _Linear_new([inputs, h], 4 * self._num_units, True)
    # i = input_gate, j = new_input, f = forget_gate, o = output_gate
    tmp, j = self._linear([inputs, h])
    i, f, o = array_ops.split(value=tmp, num_or_size_splits=3, axis=1)
...
class _Linear_new(object):
  ...
  def __init__(self,
    ...
    #print('Linear new total_arg_size', total_arg_size, 'output_size', output_size, 'shape[1]', shape[1].value)
    scope = vs.get_variable_scope()
    with vs.variable_scope(scope) as outer_scope:
      #print(shapes[0][0].value)	
      self._jweights = vs.get_variable(
          _J_VARIABLE_NAME, [shapes[0][1].value, shape[1].value],
          dtype=dtype,
          initializer=kernel_initializer)
      #note how we are changing dim
      self._weights = vs.get_variable(
          _WEIGHTS_VARIABLE_NAME, [total_arg_size, output_size - shape[1].value],
          dtype=dtype,
          initializer=kernel_initializer)
      # self._weights = vs.get_variable(
      ...
      if build_bias:
        with vs.variable_scope(outer_scope) as inner_scope:
          ...
          self._biases = vs.get_variable(
              _BIAS_VARIABLE_NAME, [output_size - shape[1].value],
              dtype=dtype,
  ...
  def __call__(self, args):
    ...
    if len(args) == 1:
      ...
      #print('Linear new', array_ops.concat(args, 1).shape, array_ops.concat(args[0], 1).shape)
      res = math_ops.matmul(array_ops.concat(args, 1), self._weights)
      j_res = math_ops.matmul(array_ops.concat(args[0], 1), self._jweights)
    if self._build_bias:
...
class _Linear(object):
      ...
      if build_bias:
        with vs.variable_scope(outer_scope) as inner_scope:
          ...
          self._biases = vs.get_variable(
              ...
              dtype=dtype,
              initializer=bias_initializer)
    #print('Linear', self._weights.shape, self._biases.shape)
  ...
  def __call__(self, args):
    ...
    if len(args) == 1:
      ...
      #print('Linear', array_ops.concat(args, 1).shape)	
      res = math_ops.matmul(array_ops.concat(args, 1), self._weights)
    if self._build_bias:
      res = nn_ops.bias_add(res, self._biases)
    return res
